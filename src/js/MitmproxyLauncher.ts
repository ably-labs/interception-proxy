...
export class MitmproxyLauncher {
  ...
  async launch(mode: InterceptionModeDTO) {
    ...
    switch (process.platform) {
      ...
      default:
        throw new Error(
          `Don’t know how to set up mitmdump interception for platform ${process.platform}`,
        );
    }
    ...
    // TODO fail if mitmproxy exits

    console.log(`Waiting for mitmdump to start`);

    let resolveResult: () => void;
    const result = new Promise<Record<string, never>>((resolve) => {
