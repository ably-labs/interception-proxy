...
export class InterceptionContext {
  ...
  private jsonRPC: TypedJSONRPCServerAndClient<
    ServerMethods,
    ClientMethods,
    ServerParams
  >;
  ...
  constructor() {
    ...
    this.jsonRPC.addMethod("injectMessage", (params) =>
      this.injectMessage(params),
    );
    this.jsonRPC.addMethod("mitmproxyReady", () =>
      this.mitmproxyLauncher?.onMitmproxyReady(),
    );
  }

  onControlWebSocketMessage(
    data: string,
    controlServerConnection: ControlServerConnection,
  ) {
  ...
  private handleTransformInterceptedMessageResponse(
    ...
    if (!this.interceptedMessagesQueue.isHead(handle)) {
      throw new Error(
        "Got response for an intercepted message that’s not at head of queue; shouldn’t be possible",
      );
    }

    const interceptedMessage = this.interceptedMessagesQueue.getHead(
      handle.predicate,
    );

    if (interceptedMessage.action !== null) {
      throw new Error(
        "Response asked us to set the action for a message that already has action set",
      );
    }
  ...
  private dequeueInterceptedMessage(predicate: InterceptedMessagePredicate) {
    console.log(
      "Dequeueing intercepted message for predicate",
      predicate.loggingDescription,
    );
    ...
    if (message.action === null) {
      throw new Error(
        `Attempted to dequeue message that doesn’t have an action: ${message}`,
      );
    }
    ...
    switch (message.action.type) {
      case "replace":
        ...
        predicate.interceptedConnection.inject(
          predicate.fromClient,
          message.action.data,
        );
        break;
      case "drop":
    ...
    if (this.interceptedMessagesQueue.hasMessages(predicate)) {
      ...
      predicate.interceptedConnection.setKeepConnectionAlive(
        false,
        predicate.fromClient
          ? WhichConnection.ToServer
          : WhichConnection.ToClient,
      );
    }
  }
  ...
  private transformNextMessage(predicate: InterceptedMessagePredicate) {
    ...
    Promise.resolve(
      this.jsonRPC.request("transformInterceptedMessage", paramsDTO),
    )
      .then((resultDTO) => {
        const handle = new InterceptedMessageHandle(
          predicate,
  ...
  enqueueMessage(
    ...
    interceptedConnection: InterceptedConnection,
  ) {
    ...
    const predicate = new InterceptedMessagePredicate(
      interceptedConnection,
      message.fromClient,
    );
  ...
  injectMessage(paramsDTO: InjectMessageParamsDTO): InjectMessageResultDTO {
    ...
    const interceptedConnection = this.proxyServer!.getInterceptedConnection(
      params.connectionID,
    );
