...
export class ProxyServer {
  ...
  start() {
    this.wss.on("connection", (clientConnection, req) => {
      ...
      if (host === undefined) {
        console.error(
          "Connection to proxy server without Ably-Test-Host header; closing",
        );
      ...
      if (proto === undefined) {
        console.error(
          "Connection to proxy server without Ably-Test-Proto header; closing",
        );
        clientConnection.close();
        return;
      }
      ...
      this.interceptedConnections.set(
        interceptedConnection.id,
        interceptedConnection,
      );
    });

    console.log(
      `Started interception proxy server to receive traffic from mitmproxy on port ${port}`,
    );
  }
