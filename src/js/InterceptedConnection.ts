...
import {
  ...
  InterceptedConnectionState,
  InterceptedConnectionEvent,
} from "./StateMachine";
...
export class InterceptedConnection {
  ...
  constructor(
    ...
    this.log(
      `New connection to proxy server (forwarded host ${host}, forwarded proto ${proto})`,
    );
  ...
  log(message: string) {
    console.log(
      `Connection ${this.id} (state ${InterceptedConnectionState[this.state]}): ${message}`,
    );
  }

  on(event: InterceptedConnectionEvent) {
    const transition = stateMachineDefinition.fetchTransition(
      this.state,
      event,
    );
    ...
    this.log(
      `Transitioning to state ${InterceptedConnectionState[transition.newState]}`,
    );
  ...
  setKeepConnectionAlive(keepAlive: boolean, connection: WhichConnection) {
    this.log(
      `set ${keepAlive ? "" : "no "}keepAlive connection ${WhichConnection[connection]}`,
    );
  ...
  private tryPropagateClosure(targetConnectionDescription: WhichConnection) {
    ...
    if (propagate) {
      this.log(
        `Propagating close of connection ${WhichConnection[targetConnectionDescription]}`,
      );
  ...
  private enqueueForSend(fromClient: boolean, data: WebSocketMessageData) {
    const queue = fromClient
      ? this.messageQueues.toServer
      : this.messageQueues.toClient;
    queue.push(data);
  }
  ...
  private tryFlushSendQueue(fromClient: boolean) {
    ...
    switch (whenCanSend) {
      case "now": {
        this.log(
          `Sending ${queue.length} injected messages to ${fromClient ? "server" : "client"}`,
        );
        const outgoingConnection = fromClient
    ...
    if (clearQueue) {
      ...
      this.tryPropagateClosure(
        fromClient ? WhichConnection.ToServer : WhichConnection.ToClient,
      );
    }
  }
